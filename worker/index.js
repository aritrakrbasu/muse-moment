// Cloudflare Worker - Muse Moment AI Service
// Handles all Gemini API calls with server-side prompts and fallbacks
...
export default {
  async fetch(request, env) {
    ...
    try {
      ...
      const { turn, total, setup, history = [], regenSeed = 0 } = body;

      const card = await generateKinkyCard(turn, total, setup, history, regenSeed, env.GEMINI_API_KEY);
...
const getSystemPromptForMode = (mode, man, woman, history, regenSeed, progress, players = []) => {
...
async function generateKinkyCard(turn, total, setup, history, regenSeed, apiKey) {
  ...
  try {
    ...
    const result = await model.generateContent({
      ...
      generationConfig: {
        temperature: Math.min(2.0, 1.1 + regenSeed * 0.1),
        maxOutputTokens: 45,
        topP: 0.9,
      },
      safetySettings: [
    ...
    // BIOLOGY & IDENTITY GUARD
    if (receiver === man) text = text.replace(/vagina|clit|pussy/gi, "cock");
    if (receiver === woman) text = text.replace(/cock|shaft|balls|frenulum/gi, "vagina");
    ...
    if (mode === "couples") {
      ...
      if (progress <= 0.15) {
        ...
        const climaxPositions = ["Missionary", "Doggy style", "Cowgirl", "Lotus"];
